...
\ea{
We consider the classic-IC FT simulation to be in its evolved, relaxed state once a rolling temporal average of $\Delta T$ over a couple hundred of freefall time units converges to within 1\% of its final value.
We run our classic-IC FT simulations for at least a thousand $\tau_{\text{ff, ev}}$ past this point to ensure that the simulations are converged, and we gather statistics from the latest, most-converged simulation times that are available.
...
\ea{
If we were to run each of these simulations for multiple thermal diffusion times, and gather PDF statistics over thousands of freefall times, we would expect to find near-perfect agreement between the PDFs of the different simulations (except were boundary conditions cause differences between FT and TT simulations).
However, the good agreement between the PDFs of each of these simulations over our modest sampling windows suggests that the dynamics in each of these distinct simulations are being drawn from the same parent statistical distribution.
}
