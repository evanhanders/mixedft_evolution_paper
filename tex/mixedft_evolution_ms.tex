...
We will refer to the common choice of fixing temperature at both boundaries as ``TT'' boundaries, and fixing the flux at both boundaries as ``FF'' boundaries\footnote{Note, in ref.~\cite{goluskin2016}, our TT, FF, and FT are repsectively called RB1, RB2, and RB3.}.
...
The thermal relaxation of FT simulations is analagous to a sweep through parameter space in which dynamics are sampled over a range of values of Ra.
...
We study incompressible  under a freefall nondimensionalization; for details of this nondimensionalization, we refer readers to our previous work \cite{anders&all2018}.
...
In nonrotating RBC (section \ref{sec:2d_results}), we set Ek$\,= \infty$.
...
\caption{ 
	...
	However, in the FT case, the temperature anomaly of the cold plume is much larger than the hot plume, which does not appear on this colorscale.
...
We interpolate the (unevenly spaced) vertical Chebyshev gridpoints onto an evenly spaced grid before histogramming the flow values into 200 bins and creating the PDFs.
...
At late times (lower right), the FT simulation's PDF is indistinguishable from the TT PDF between the cold (left), fixed-tempertaure boundary and the modal value.
From the modal value towards warmer temperatures, we find that fixed-flux (lower boundary) is capable of producing more extreme temperature events and results in a more extended PDF tail.
...
\caption{ 
	(Left three panels) Time traces of scalar quantities in an FT (orange) and TT (purple) are shown.
...
When we split the profile into upflows and downflows, we find that FT upflows/downflows are slightly warmer/cooler than their TT counterparts at the hot, bottom boundary.
...
\end{enumerate}
We show this prcedure in practice in Fig.~\ref{fig:rbc_restart_description}.
...
These findings suggest that comparisons of systems which are nondimensionalized on the flux and the temperature may be more straightforward in a thermal diffusion nondimensionalization \cite{goluskin2016} than the freefall nondimensionalization we have used throughout this work.
...
Select TT cases are plotted as cyan circles with purple outlines (where the cyan color denotes the value of Ek$\,=\,10^{-6}$ according to the colorbar).
...
\caption{
	...
	Input quantities are the boundary conditions (BCs), Rayleigh number (Ra), coefficient resolution (nz$\times$nx$\times$ny), and the total simulation run time in freefall units $t_{\text{simulation}}$ and in cpu-hours.
...
\hline																	
BCs	&	Ra	&	nz$\times$nx$\times$ny	& $t_{\text{simulation}}$	&	cpu-hours 	&	Nu	&	Nu comp	&	Pe  & Ro \\
\hline \hline \multicolumn{6}{c}{\vspace{-0.2cm}}\\
