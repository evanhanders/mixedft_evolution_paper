...
\ea{In our $\Gamma = 2$ box, the smallest wavenumber permitted is $k = \pi$, and at that wavenumber the critical values of are Ra$_{\partial_z T} = 1357.57$ for FT boundaries and Ra$_{\Delta T} = 1707.94$ for TT boundaries, which are slightly larger than the classical onset values}.
...
Timestepping through the thermal relaxation of a classic FT simulation is prohibatively expensive in 3D, even for our least turbulent Ra$_{\partial_z T} = 2.61 \times 10^9$ case.
...
This classic FT 2D case took $1.21 \times 10^5$ cpu-hours, so we estimate that a classic FT case in 3D at this value of Ra$_{\partial_z T}$ would take O(10 million) cpu-hours.
We found previously \cite{anders&all2018} at lower Ra$_{\partial_z T}$ that 3D classic FT cases exhibited the same thermal rundown as 2D cases.
As a result, we will focus here on comparisons of FT and TT simulations in the evolved state through the use of TT-to-FT initial conditions.
