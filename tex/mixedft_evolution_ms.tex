...
In practice, we find that our non-rotating simulations equilibrate in $\sim 2.5\sqrt{\text{Ra}\,\text{Pr}}/\text{Nu}$ nondimensional time units(see section \ref{sec:time_evolution_classic_ICs}), and our rotating simulation equilibrates in $\sim (2/3)\sqrt{\text{Ra}\,\text{Pr}}/\text{Nu}$ (see section \ref{sec:results_rotating}).
...
\caption{ 
	(Left three panels) Time traces of scalar quantities, which have been averaged over a rolling time window of 25 simulation time units, are shown for a simulation with Ra$_{\Delta T} = 10^9$ which starts with TT boundary conditions and then is switched to FT boundary conditions \ea{with Ra$_{\partial_z T} = 4.83 \times 10^{10}$}.
...
Timestepping through the thermal relaxation of a classic FT simulation is prohibatively expensive in 3D, even for our least turbulent case with an evolved Ra$_{\Delta T} \approx 10^8$.
In order to timestep through the intense, early convective transient, we anticipate that such a simulation would require a coefficient resolution of $\gtrsim 512^3$, a factor of 64 times more coefficients than our comparable 2D case at 2048x1024.
This 2D, classic-IC, FT case took $1.21 \times 10^5$ cpu-hours, so we estimate that a classic FT case in 3D at this value of Ra$_{\partial_z T}$ would take O(10 million) cpu-hours.
We found in previous work \cite{anders&all2018} at lower Ra$_{\partial_z T}$ that 3D classic FT cases exhibited the same thermal rundown as 2D cases.
As a result, we will focus here on 3D comparisons of FT and TT simulations in the statistically-stationary state through the use of TT-to-FT initial conditions.

We find that equilibrated FT and TT simulations are analogous in a volume-averaged sense: measurements of Nu (31.4 for TT, 31.9 for FT) and Pe ($1.78 \times 10^3$ for both) are nearly indistinguishable.
In Fig.~\ref{fig:rbc_3D_panels}, we show dynamical slices from 3D TT and TT-to-FT simulations.
As in 2D, we find no difference between the PDFs of evolved quantities like the nonlinear transport ($wT$) between TT and FT simulations.
Therefore, we have focused on the temperature fields in this figure, which does display differences between the two sets of boundary conditions.
...
These differences are interesting, \ea{can be seen visually in Fig.~\ref{fig:rbc_3D_panels},} and are explored further in \ea{the next section} (section \ref{sec:asymmetries}) and Fig.~\ref{fig:rbc_dynamics_asymmetries}, but vanish in the interior and do seem to affect the convective dynamics appreciably.
...
\caption{ 
	(Left three panels) Time traces of scalar quantities in a \ea{classic-IC} FT (orange) and TT (purple) simulation are shown.
...
We once again find it important to briefly note the difference in computational cost between the \ea{classic-IC} FT, \ea{TT-to-FT,} and TT simulations conducted here.
...
Through studying this relaxation and the relaxed states simulations, we come to the following conclusions:
\begin{enumerate}
...
The rapid evolution of \ea{all of our simulations other than the classic-IC FC} simulations suggests that RBC \ea{thermally} restratifies itself instantaneously.
...
\ea{This is principly a concern in systems like classic-IC FT simulations which can have very long relaxational times compared to dynamical times.}
...
Some authors who study astrophysical convection \citep{featherstone&hindman2016a, strugarek&all2018, bordwell&all2018} employ FF boundary conditions, and our results here suggest that such a choice may be ideal in those complex simulations.
\ea{Indeed, convection in 3D spherical shells does seem to thermally converge faster with FF boundaries than FT boundaries (Matilsky 2020, private communication).
However, FF boundaries have also been shown to drive different flow morphologies than FT or TT boundaries in rotating convection in spherical geometries \cite{matilsky&all2020, calkins&all2015}, so perhaps numericists should lean on the development of ``smarter'' initial conditions rather than changes to the boundary conditions to achieve faster relaxation for some cases. 
}
...
\caption{
	...
	Reported values of Nu, Re, and Ro are the sample mean over the last 500 freefall time units \ea{for all rotating cases and all 2D cases except the Ra$_{\partial_z T} = 9.51\times10^{11}$ TT-to-FT case, where samples were taken over 200 evolved freefall time units; samples were taken over 350 freefall time units for 3D nonrotating cases}.
