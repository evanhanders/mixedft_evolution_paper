...
\end{equation}
where $\angles{}$ represent a volume average ($\angles{\ea{s}} \equiv \iint \ea{s}\,dx\,dz / \Gamma$ in 2D and $\angles{\ea{s}} \equiv \iiint \ea{s}\,dx\,dy\,dz / \Gamma^2$ in 3D for some \ea{scalar} quantity \ea{$s$}), and $\Delta T = \angles{\partial_z T}$ is the (negative) temperature difference between the top and bottom plate.
...
\end{equation}
where $|\bm{\ea{q}}|$ represents the magnitude of the vector \ea{quantity $\bm{q}$}.
...
We assume zero velocity in the initial state, except in the case of TT-to-FT simulations, where velocities are taken directly from the \ea{corresponding} TT simulation and scaled according to Eqn.~\ref{eqn:ra_relation}.
...
The thermal energy reservoir of our 3D Cartesian systems and the rate of change of this reservoir due to conduction at the boundaries \ea{are derived from volume-integrals of the temperature and vertical fluxes,}
\begin{equation}
E = \ea{\iiint_V} T\,dx\,dy\,dz = \Gamma^2 \ea{\int_{z=-0.5}^{z=0.5}} T\,dz,
\qquad
\frac{dE}{dt} = -\ea{\iiint_V} \grad\cdot \bm{F}_{\text{cond}}\,dx\,dy\,dz = -\Gamma^2 F_{\text{cond,z}}\bigg|_{z=-0.5}^{z=0.5}
\end{equation}
...
In this relaxed state, the supply of warm fluid from the bottom plume and cold fluid from the top plume come into balance, and the FT dynamics (bottom right) are indistinguishable from \ea{the} TT dynamics (top right).
...
The time evolution of FT simulations with Nu-based ICs is similar to the time evolution of TT simulations with classic ICs, with a few \ea{small differences}.
...
In short, we find that \ea{simulations with mixed thermal boundary conditions} can experience a long thermal relaxation which is not experienced by \ea{simulations with two fixed-temperature boundaries; furthermore,} to first order, \ea{mixed thermal boundary conditions} do not introduce important asymmetries into the solution.
