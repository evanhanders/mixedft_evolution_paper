...
We find that when classic initial conditions which are in hydrostatic and thermal equilibrium are employed, the thermal relaxation of FT systems is very long compared to TT systems, in which it is nearly instantaneous.
...
We also find that this long thermal relaxation can be bypassed by constructing smarter initial conditions based on the expected evolved value of Nu, or by simply using the results of TT simulations as initial conditions for FT simulations.
...
The temperature scale, $\Delta$, is set by either the temperature jump across the domain ($\Delta = \Delta T_0 =  T_0(z=0.5)-T_0(z=-0.5)$) for TT boundaries or by the temperature gradient lengthscale ($\Delta = L_z \partial_z T_0$) for FT boundaries.
...
\end{equation}
where $\angles{}$ represent a volume average ($\angles{s} \equiv \iint s\,dx\,dz / \Gamma$ in 2D and $\angles{s} \equiv \iiint s\,dx\,dy\,dz / \Gamma^2$ in 3D for some scalar quantity $s$), and $\Delta T = \angles{\partial_z T}$ is the (negative) temperature difference between the top and bottom plate.
...
\end{equation}
where $|\bm{q}|$ represents the magnitude of the vector quantity $\bm{q}$.
...
In FT simulations, the evolved, nonlinear dynamics determine the magnitude of $\Delta T$.
As a result, the time evolution of FT simulations is sensitive to how well the initial conditions guess the evolved $\Delta T$.
...
\begin{equation}
\frac{\partial T_N}{\partial z} = (\grad T)_{\text{interior}} - \xi(z)[1 + (\grad T)_{\text{interior}}].
...
Discontinuous profiles are unstable in our spectral methods, so we utilize a smooth windowing function, $\xi(z)$, to set the temperature gradient to $-1$ near the boundaries,
\begin{equation}
...
\end{equation}
Here, $\delta_\xi = -\Delta T / 2$ is an estimate of the boundary layer width and the temperature gradient in the interior of the domain, $(\grad T)_{\text{interior}}$, is determined by setting $\int \partial_z T_N dz = \Delta T$.
...
We assume zero velocity in the initial state, except in the case of TT-to-FT simulations, where velocities are taken directly from the corresponding TT simulation and scaled according to Eqn.~\ref{eqn:ra_relation}.
...
To ensure accurate comparisons, we measure flow statistics over multiples of $\tau_{\text{ff, ev}}$ rather than over multiples of the nondimensional time units.


The thermal energy reservoir of our 3D Cartesian systems and the rate of change of this reservoir due to conduction at the boundaries are derived from volume-integrals of the temperature and vertical fluxes,
\begin{equation}
E = \iiint_V T\,dx\,dy\,dz = \Gamma^2 \int_{z=-0.5}^{z=0.5} T\,dz,
\qquad
\frac{dE}{dt} = -\iiint_V \grad\cdot \bm{F}_{\text{cond}}\,dx\,dy\,dz = -\Gamma^2 F_{\text{cond,z}}\bigg|_{z=-0.5}^{z=0.5}
\end{equation}
...
In this relaxed state, the supply of warm fluid from the bottom plume and cold fluid from the top plume come into balance, and the FT dynamics (bottom right) are indistinguishable from the TT dynamics (top right).
...
\caption{ 
	...
	All traces have been averaged over a rolling window of 100 simulation time units to increase the clarity of the evolutionary trend.
...
The time evolution of FT simulations with Nu-based ICs is similar to the time evolution of TT simulations with classic ICs, with a few small differences.
...
Per table \ref{table:pdf_values}, the evolved states of FT simulations with Nu-based ICs and classic ICs are very similar.
...
\begin{figure}[p!]
\includegraphics[width=\textwidth]{./rbc_3D_panels.pdf}
\caption{ 
...
\caption{ 
	...
	(Right six panels) The three panels on the left display the bottom boundary layers while the right panels display the top boundary layers.
...
In Fig.~\ref{fig:rbc_dynamics_asymmetries}, we examine the dynamical nature of the asymmetries that the FT boundaries introduce into the simulation near the fixed-flux boundary.
...
In short, we find that simulations with mixed thermal boundary conditions can experience a long thermal relaxation which is not experienced by simulations with two fixed-temperature boundaries; furthermore, to first order, mixed thermal boundary conditions do not introduce important asymmetries into the solution.
...
However, for most convective studies where the lifetime of the natural convective system is much larger than its Kelvin-Helmholtz timescale (the time it would take for an astrophysical body to radiate its full gravitational potential energy given its current luminosity \cite{carroll&ostlie}), it is essential to study relaxed convection, and our results point towards the importance of either choosing good initial conditions (TT or TT-to-FT simulations) or running simulations to thermal relaxation.
...
RBC is fundamentally symmetrical, but many natural convective processes occur in density-stratified domains in which the symmetries of the problem are broken.
...
\caption{
	...
	Input quantities are the boundary conditions (BCs), initial conditions (ICs), Rayleigh number (Ra), coefficient resolution (nx$\times$ny$\times$nz, or horizontal $\times$ vertical), and the total simulation run time in freefall units $t_{\text{simulation}}$ and in cpu-hours.
	Output quantities are the Nusselt (Nu), P\'{e}clet (Pe), and Rossby (Ro) numbers.
	Reported values of Nu, Re, and Ro are the sample mean over the last 500 freefall time units for all rotating cases and all 2D cases except the Ra$_{\partial_z T} = 9.51\times10^{11}$ TT-to-FT case, where samples were taken over 200 evolved freefall time units; samples were taken over 350 freefall time units for 3D non-rotating cases.
...
\bibitem [{\citenamefont {{Ishiwatari}}\ \emph {et~al.}(1994)\citenamefont
  {{Ishiwatari}}, \citenamefont {{Takehiro}},\ and\ \citenamefont
  {{Hayashi}}}]{ishiwatari&all1994}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {M.}~\bibnamefont
  {{Ishiwatari}}}, \bibinfo {author} {\bibfnamefont {S.}~\bibnamefont
  {{Takehiro}}}, \ and\ \bibinfo {author} {\bibfnamefont {Y.~Y.}\ \bibnamefont
  {{Hayashi}}},\ }\bibfield  {title} {\enquote {\bibinfo {title} {{The effects
  of thermal conditions on the cell sizes of two-dimensional convection}},}\
  }\href {\doibase 10.1017/S0022112094003022} {\bibfield  {journal} {\bibinfo
  {journal} {Journal of Fluid Mechanics}\ }\textbf {\bibinfo {volume} {281}},\
  \bibinfo {pages} {33--50} (\bibinfo {year} {1994})}\BibitemShut {NoStop}%
...
\bibitem [{\citenamefont {{Chapman}}\ and\ \citenamefont
  {{Proctor}}(1980)}]{chapman&proctor1980}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {C.~J.}\ \bibnamefont
  {{Chapman}}}\ and\ \bibinfo {author} {\bibfnamefont {M.~R.~E.}\ \bibnamefont
  {{Proctor}}},\ }\bibfield  {title} {\enquote {\bibinfo {title} {{Nonlinear
  Rayleigh-Benard convection between poorly conducting boundaries}},}\ }\href
  {\doibase 10.1017/S0022112080001917} {\bibfield  {journal} {\bibinfo
  {journal} {Journal of Fluid Mechanics}\ }\textbf {\bibinfo {volume} {101}},\
  \bibinfo {pages} {759--782} (\bibinfo {year} {1980})}\BibitemShut {NoStop}%
...
\bibitem [{\citenamefont {{Burns}}\ \emph {et~al.}(2020)\citenamefont
  {{Burns}}, \citenamefont {{Vasil}}, \citenamefont {{Oishi}}, \citenamefont
  {{Lecoanet}},\ and\ \citenamefont {{Brown}}}]{burns&all2020}%
  \BibitemOpen
  ...
  with spectral methods}},}\ }\href {\doibase 10.1103/PhysRevResearch.2.023068}
  {\bibfield  {journal} {\bibinfo  {journal} {Physical Review Research}\
  }\textbf {\bibinfo {volume} {2}},\ \bibinfo {eid} {023068} (\bibinfo {year}
  {2020})},\ \Eprint {http://arxiv.org/abs/1905.10388} {arXiv:1905.10388
...
\bibitem [{\citenamefont {{Calkins}}\ \emph {et~al.}(2015)\citenamefont
  {{Calkins}}, \citenamefont {{Hale}}, \citenamefont {{Julien}}, \citenamefont
  {{Nieves}}, \citenamefont {{Driggs}},\ and\ \citenamefont
  {{Marti}}}]{calkins&all2015}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {Michael~A.}\
  \bibnamefont {{Calkins}}}, \bibinfo {author} {\bibfnamefont {Kevin}\
  \bibnamefont {{Hale}}}, \bibinfo {author} {\bibfnamefont {Keith}\
  \bibnamefont {{Julien}}}, \bibinfo {author} {\bibfnamefont {David}\
  \bibnamefont {{Nieves}}}, \bibinfo {author} {\bibfnamefont {Derek}\
  \bibnamefont {{Driggs}}}, \ and\ \bibinfo {author} {\bibfnamefont {Philippe}\
  \bibnamefont {{Marti}}},\ }\bibfield  {title} {\enquote {\bibinfo {title}
  {{The asymptotic equivalence of fixed heat flux and fixed temperature thermal
  boundary conditions for rapidly rotating convection}},}\ }\href {\doibase
  10.1017/jfm.2015.606} {\bibfield  {journal} {\bibinfo  {journal} {Journal of
  Fluid Mechanics}\ }\textbf {\bibinfo {volume} {784}},\ \bibinfo {eid} {R2}
  (\bibinfo {year} {2015})},\ \Eprint {http://arxiv.org/abs/1507.07168}
  {arXiv:1507.07168 [physics.flu-dyn]} \BibitemShut {NoStop}%
\bibitem [{\citenamefont {{Zhu}}\ \emph {et~al.}(2018)\citenamefont {{Zhu}},
  ...
  \BibitemShut {NoStop}%
\bibitem [{\citenamefont {{King}}\ \emph {et~al.}(2009)\citenamefont {{King}},
  \citenamefont {{Stellmach}}, \citenamefont {{Noir}}, \citenamefont
  {{Hansen}},\ and\ \citenamefont {{Aurnou}}}]{king&all2009}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {Eric~M.}\ \bibnamefont
  {{King}}}, \bibinfo {author} {\bibfnamefont {Stephan}\ \bibnamefont
  {{Stellmach}}}, \bibinfo {author} {\bibfnamefont {Jerome}\ \bibnamefont
  {{Noir}}}, \bibinfo {author} {\bibfnamefont {Ulrich}\ \bibnamefont
  {{Hansen}}}, \ and\ \bibinfo {author} {\bibfnamefont {Jonathan~M.}\
  \bibnamefont {{Aurnou}}},\ }\bibfield  {title} {\enquote {\bibinfo {title}
  {{Boundary layer control of rotating convection systems}},}\ }\href {\doibase
  10.1038/nature07647} {\bibfield  {journal} {\bibinfo  {journal} {\nat}\
  }\textbf {\bibinfo {volume} {457}},\ \bibinfo {pages} {301--304} (\bibinfo
  {year} {2009})}\BibitemShut {NoStop}%
\bibitem [{\citenamefont {{Schmitz}}\ and\ \citenamefont
  {{Tilgner}}(2009)}]{schmitz&tilgner2009}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {S.}~\bibnamefont
  {{Schmitz}}}\ and\ \bibinfo {author} {\bibfnamefont {A.}~\bibnamefont
  {{Tilgner}}},\ }\bibfield  {title} {\enquote {\bibinfo {title} {{Heat
  transport in rotating convection without Ekman layers}},}\ }\href {\doibase
  10.1103/PhysRevE.80.015305} {\bibfield  {journal} {\bibinfo  {journal}
  {\pre}\ }\textbf {\bibinfo {volume} {80}},\ \bibinfo {eid} {015305} (\bibinfo
  {year} {2009})}\BibitemShut {NoStop}%
\bibitem [{\citenamefont {{Zhong}}\ and\ \citenamefont
  {{Ahlers}}(2010)}]{zhong&ahlers2010}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {Jin-Qiang}\
  \bibnamefont {{Zhong}}}\ and\ \bibinfo {author} {\bibfnamefont {Guenter}\
  \bibnamefont {{Ahlers}}},\ }\bibfield  {title} {\enquote {\bibinfo {title}
  {{Heat transport and the large-scale circulation in rotating turbulent
  Rayleigh-B{\'e}nard convection}},}\ }\href {\doibase
  10.1017/S002211201000399X} {\bibfield  {journal} {\bibinfo  {journal}
  {Journal of Fluid Mechanics}\ }\textbf {\bibinfo {volume} {665}},\ \bibinfo
  {pages} {300--333} (\bibinfo {year} {2010})}\BibitemShut {NoStop}%
...
\bibitem [{\citenamefont {{Stevens}}\ \emph {et~al.}(2013)\citenamefont
  ...
  {{Lohse}}}]{stevens&all2013}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {Richard J.~A.~M.}\
  \bibnamefont {{Stevens}}}, \bibinfo {author} {\bibfnamefont {Herman J.~H.}\
  \bibnamefont {{Clercx}}}, \ and\ \bibinfo {author} {\bibfnamefont {Detlef}\
  \bibnamefont {{Lohse}}},\ }\bibfield  {title} {\enquote {\bibinfo {title}
  {{Heat transport and flow structure in rotating Rayleigh-B{\'e}nard
  convection}},}\ }\href {\doibase 10.1016/j.euromechflu.2013.01.004}
  {\bibfield  {journal} {\bibinfo  {journal} {European Journal of Mechanics B
  Fluids}\ }\textbf {\bibinfo {volume} {40}},\ \bibinfo {pages} {41--49}
  (\bibinfo {year} {2013})},\ \Eprint {http://arxiv.org/abs/1204.3396}
  {arXiv:1204.3396 [physics.flu-dyn]} \BibitemShut {NoStop}%
\bibitem [{\citenamefont {{Ecke}}(2015)}]{ecke2015}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {Robert~E.}\
  \bibnamefont {{Ecke}}},\ }\bibfield  {title} {\enquote {\bibinfo {title}
  {{Scaling of heat transport near onset in rapidly rotating convection}},}\
  }\href {\doibase 10.1016/j.physleta.2015.06.053} {\bibfield  {journal}
  {\bibinfo  {journal} {Physics Letters A}\ }\textbf {\bibinfo {volume}
  {379}},\ \bibinfo {pages} {2221--2223} (\bibinfo {year} {2015})}\BibitemShut
  {NoStop}%
\bibitem [{\citenamefont {{Grooms}}(2015)}]{grooms2015}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {Ian}\ \bibnamefont
  {{Grooms}}},\ }\bibfield  {title} {\enquote {\bibinfo {title} {{Asymptotic
  behavior of heat transport for a class of exact solutions in rotating
  Rayleigh-B{\'e}nard convection}},}\ }\href {\doibase
  10.1080/03091929.2015.1036054} {\bibfield  {journal} {\bibinfo  {journal}
  {Geophysical and Astrophysical Fluid Dynamics}\ }\textbf {\bibinfo {volume}
  {109}},\ \bibinfo {pages} {145--158} (\bibinfo {year} {2015})},\ \Eprint
  {http://arxiv.org/abs/1403.0658} {arXiv:1403.0658 [physics.flu-dyn]}
  \BibitemShut {NoStop}%
\bibitem [{\citenamefont {{Grooms}}\ and\ \citenamefont
  {{Whitehead}}(2015)}]{grooms&whitehead2015}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {Ian}\ \bibnamefont
  {{Grooms}}}\ and\ \bibinfo {author} {\bibfnamefont {Jared~P.}\ \bibnamefont
  {{Whitehead}}},\ }\bibfield  {title} {\enquote {\bibinfo {title} {{Bounds on
  heat transport in rapidly rotating Rayleigh-B{\'e}nard convection}},}\ }\href
  {\doibase 10.1088/0951-7715/28/1/29} {\bibfield  {journal} {\bibinfo
  {journal} {Nonlinearity}\ }\textbf {\bibinfo {volume} {28}},\ \bibinfo {eid}
  {29} (\bibinfo {year} {2015})},\ \Eprint {http://arxiv.org/abs/1405.1458}
  {arXiv:1405.1458 [physics.flu-dyn]} \BibitemShut {NoStop}%
\bibitem [{\citenamefont {{Rubio}}\ \emph {et~al.}(2014)\citenamefont
  {{Rubio}}, \citenamefont {{Julien}}, \citenamefont {{Knobloch}},\ and\
  \citenamefont {{Weiss}}}]{rubio&all2014}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {Antonio~M.}\
  \bibnamefont {{Rubio}}}, \bibinfo {author} {\bibfnamefont {Keith}\
  \bibnamefont {{Julien}}}, \bibinfo {author} {\bibfnamefont {Edgar}\
  \bibnamefont {{Knobloch}}}, \ and\ \bibinfo {author} {\bibfnamefont
  {Jeffrey~B.}\ \bibnamefont {{Weiss}}},\ }\bibfield  {title} {\enquote
  {\bibinfo {title} {{Upscale Energy Transfer in Three-Dimensional Rapidly
  Rotating Turbulent Convection}},}\ }\href {\doibase
  10.1103/PhysRevLett.112.144501} {\bibfield  {journal} {\bibinfo  {journal}
  {\prl}\ }\textbf {\bibinfo {volume} {112}},\ \bibinfo {eid} {144501}
  (\bibinfo {year} {2014})}\BibitemShut {NoStop}%
\bibitem [{\citenamefont {{Guervilly}}\ \emph {et~al.}(2014)\citenamefont
  {{Guervilly}}, \citenamefont {{Hughes}},\ and\ \citenamefont
  {{Jones}}}]{guervilly&all2014}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {C{\'e}line}\
  \bibnamefont {{Guervilly}}}, \bibinfo {author} {\bibfnamefont {David~W.}\
  \bibnamefont {{Hughes}}}, \ and\ \bibinfo {author} {\bibfnamefont {Chris~A.}\
  \bibnamefont {{Jones}}},\ }\bibfield  {title} {\enquote {\bibinfo {title}
  {{Large-scale vortices in rapidly rotating Rayleigh-B{\'e}nard
  convection}},}\ }\href {\doibase 10.1017/jfm.2014.542} {\bibfield  {journal}
  {\bibinfo  {journal} {Journal of Fluid Mechanics}\ }\textbf {\bibinfo
  {volume} {758}},\ \bibinfo {pages} {407--435} (\bibinfo {year} {2014})},\
  \Eprint {http://arxiv.org/abs/1403.7442} {arXiv:1403.7442 [physics.flu-dyn]}
  \BibitemShut {NoStop}%
\bibitem [{\citenamefont {{Guervilly}}\ and\ \citenamefont
  {{Hughes}}(2017)}]{guervilly&hughes2017}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {C{\'e}line}\
  \bibnamefont {{Guervilly}}}\ and\ \bibinfo {author} {\bibfnamefont
  {David~W.}\ \bibnamefont {{Hughes}}},\ }\bibfield  {title} {\enquote
  {\bibinfo {title} {{Jets and large-scale vortices in rotating
  Rayleigh-B{\'e}nard convection}},}\ }\href {\doibase
  10.1103/PhysRevFluids.2.113503} {\bibfield  {journal} {\bibinfo  {journal}
  {Physical Review Fluids}\ }\textbf {\bibinfo {volume} {2}},\ \bibinfo {eid}
  {113503} (\bibinfo {year} {2017})},\ \Eprint
  {http://arxiv.org/abs/1711.03030} {arXiv:1711.03030 [physics.flu-dyn]}
  \BibitemShut {NoStop}%
\bibitem [{\citenamefont {{Favier}}\ \emph {et~al.}(2014)\citenamefont
  {{Favier}}, \citenamefont {{Silvers}},\ and\ \citenamefont
  {{Proctor}}}]{favier&all2014}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {B.}~\bibnamefont
  {{Favier}}}, \bibinfo {author} {\bibfnamefont {L.~J.}\ \bibnamefont
  {{Silvers}}}, \ and\ \bibinfo {author} {\bibfnamefont {M.~R.~E.}\
  \bibnamefont {{Proctor}}},\ }\bibfield  {title} {\enquote {\bibinfo {title}
  {{Inverse cascade and symmetry breaking in rapidly rotating Boussinesq
  convection}},}\ }\href {\doibase 10.1063/1.4895131} {\bibfield  {journal}
  {\bibinfo  {journal} {Physics of Fluids}\ }\textbf {\bibinfo {volume} {26}},\
  \bibinfo {eid} {096605} (\bibinfo {year} {2014})},\ \Eprint
  {http://arxiv.org/abs/1408.6483} {arXiv:1408.6483 [physics.flu-dyn]}
  \BibitemShut {NoStop}%
\bibitem [{\citenamefont {{Favier}}\ \emph {et~al.}(2019)\citenamefont
  {{Favier}}, \citenamefont {{Guervilly}},\ and\ \citenamefont
  {{Knobloch}}}]{favier&all2019}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {Benjamin}\
  \bibnamefont {{Favier}}}, \bibinfo {author} {\bibfnamefont {C{\'e}line}\
  \bibnamefont {{Guervilly}}}, \ and\ \bibinfo {author} {\bibfnamefont {Edgar}\
  \bibnamefont {{Knobloch}}},\ }\bibfield  {title} {\enquote {\bibinfo {title}
  {{Subcritical turbulent condensate in rapidly rotating Rayleigh-B{\'e}nard
  convection}},}\ }\href {\doibase 10.1017/jfm.2019.58} {\bibfield  {journal}
  {\bibinfo  {journal} {Journal of Fluid Mechanics}\ }\textbf {\bibinfo
  {volume} {864}},\ \bibinfo {eid} {R1} (\bibinfo {year} {2019})},\ \Eprint
  {http://arxiv.org/abs/1901.04269} {arXiv:1901.04269 [physics.flu-dyn]}
  \BibitemShut {NoStop}%
\bibitem [{\citenamefont {{Couston}}\ \emph {et~al.}(2020)\citenamefont
  {{Couston}}, \citenamefont {{Lecoanet}}, \citenamefont {{Favier}},\ and\
  \citenamefont {{Le Bars}}}]{couston&all2020}%
  \BibitemOpen
  ...
  {{Shape and size of large-scale vortices: A generic fluid pattern in
  geophysical fluid dynamics}},}\ }\href {\doibase
  10.1103/PhysRevResearch.2.023143} {\bibfield  {journal} {\bibinfo  {journal}
  {Physical Review Research}\ }\textbf {\bibinfo {volume} {2}},\ \bibinfo {eid}
  {023143} (\bibinfo {year} {2020})},\ \Eprint
  {http://arxiv.org/abs/1909.03244} {arXiv:1909.03244 [physics.flu-dyn]}
  \BibitemShut {NoStop}%
...
\bibitem [{\citenamefont {Carroll}\ and\ \citenamefont
  {Ostlie}(2017)}]{carroll&ostlie}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {B.W.}\ \bibnamefont
  {Carroll}}\ and\ \bibinfo {author} {\bibfnamefont {D.A.}\ \bibnamefont
  {Ostlie}},\ }\href@noop {} {\emph {\bibinfo {title} {An Introduction to
  Modern Astrophysics}}}\ (\bibinfo  {publisher} {Cambridge University Press},\
